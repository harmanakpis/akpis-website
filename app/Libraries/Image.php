...
class Image {
    ...
    function __construct() {
        $image['base_path'] = ROOTPATH . 'assets/thumb';
        $image['base_url'] = 'assets/thumb';
        $image['sharpen'] = TRUE;
    ...
    public function rimg($source, $params, $no_cache = FALSE) {
        ...
        if (file_exists($this->config['image']['base_path'] . '/' . $source)) {
            $source = $this->config['image']['base_path'] . '/' . $source;
        } else {
        ...
        if (!empty($params['title'])) {
            $extra_parameters .= "title=\"{$params['title']}\"";
        }


        if (!is_file($source)) {
            if (isset($params['bgcss']) && $params['bgcss']) {
                return 'background-image:url(https://dummyimage.com/' . $params['width'] . 'x' . $params['height'] . '&text=' . $params['alt'] . ') !important';
            }
            if (isset($params['return']) && $params['return']) {
                return 'https://dummyimage.com/' . $params['width'] . 'x' . $params['height'] . '&text=' . $params['alt'];
            }
            return '<img src="https://dummyimage.com/' . $params['width'] . 'x' . $params['height'] . '&text=' . $params['alt'] . '" width="' . $params['width'] . '" height="' . $params['height'] . '" alt="noimage ' . $source . '" '.$extra_parameters.'>';
        }
        ...
        if (!is_dir($this->config['image']['base_path'] . $dir)) {
            if (is_writable($this->config['image']['base_path'])) {
                mkdir($this->config['image']['base_path'] . $dir);
            } else {
                die('Unable to create cache dir in ' . $this->config['image']['base_path']);
            }
        }

        // full path to final file
        $dst['file'] = $this->config['image']['base_path'] . $dir . "/" . basename($source);
        ...
        if ($src['width'] === $dst['width'] && $src['height'] === $dst['height']) {
            ...
            } else if (isset($params['bgcss']) && $params['bgcss']) {
                return 'background-image:url(' . base_url($temp['dirname'] . "/" . basename($dst['file'])) . ') !important';
            } else {
        ...
        if (file_exists($dst['file']) AND $params['nocache'] !== TRUE) {
            ...
            if ($params['return']) {
                return "{$this->config['image']['base_url']}{$dir}/" . basename($dst['file']);
            } else if (isset($params['bgcss']) && $params['bgcss']) {
                return 'background-image:url(' . "{$this->config['image']['base_url']}{$dir}/" . basename($dst['file']) . ') !important';
            } else {
                return "<img src=\"{$this->config['image']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$params['width']}\" height=\"{$params['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
            }
        }
        ...
        if ($params['return']) {
            ...
            return "<img src=\"{$this->config['image']['base_url']}{$dir}/" . basename($dst['file']) . "\" width=\"{$dst['width']}\" height=\"{$dst['height']}\" alt=\"{$params['alt']}\" {$extra_parameters}/>";
        }
    }
